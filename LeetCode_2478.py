...
class Solution3:
    def beautifulPartitions(self, s: str, k: int, minLength: int) -> int:
        ...
        With prefix sum (pay attention, we only collect the sum at index i if
        s[i] is a non-prime and s[i + 1] is a prime, indicating that s[i] can
        be an end to a previous partition), the runtime improves to O(NK).

        1119 ms, faster than 88.36%
        """
        primes = {'2', '3', '5', '7'}
        if s[0] not in primes or s[-1] in primes:
            return 0
        ...
        # Note that we use a 1D DP here. The base case is i == 1
        dp = [int(s[j] not in primes and j + 1 >= minLength) for j in range(N)]
        # Create a prefix sum where we only add value if s[i] points to a non-
        # prime and s[i + 1] points to a prime. This way, presum[t] is the sum
        # of all possible previous partitions ending at some index before t.
        presum = [0]
        for j in range(1, N - 1):
            if s[j] not in primes and s[j + 1] in primes:
                presum.append(presum[-1] + dp[j])
            else:
        ...
        for i in range(2, k + 1):
            ...
            for j in range(minLength * i - 1, N):
                ...
                if s[j] not in primes:
                    tmp[j] = presum[j - minLength]
                    if j + 1 < N and s[j + 1] in primes:
                        pretmp[j] += tmp[j]
            dp = tmp
            presum = pretmp
        return dp[-1] % MOD
