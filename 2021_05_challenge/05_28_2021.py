...
class Solution3:
    def maximumUniqueSubarray(self, nums: List[int]) -> int:
        ...
        """
        cumsum = list(itertools.accumulate(nums))
        idx_map = collections.defaultdict(lambda: -1)
        l = 0
        res = 0
        for r in range(len(nums)):
            val = nums[r]
            pre_i = idx_map[val]  # obtain the idx of the most recent occurrence
            if pre_i >= l:
                l = pre_i + 1
            res = max(res, cumsum[r] - (cumsum[l - 1] if l > 0 else 0))
            idx_map[val] = r  # update the idx of the most recent occurrence
        return res
