...
class Solution1:
    def numMatchingSubseq(self, s: str, words: List[str]) -> int:
        ...
        I remembered this one, but not the solution. I remembered that LCS did
        not work. I tried it again, and surely it would time out. The current
        solution is based on backtracking and binary search. We find the
        indices of all the letters in s. Then for each letter in words, we
        iterate through all the matching indices from s, and then backtrack
        from there. We use binary search to speed up the index location of the
        next letter.

        O(W(N + logN)), 3367 ms, faster than 11.33% 
        """
        N = len(s)
        
        @lru_cache(maxsize=None)
        def is_match(wi: int, si: int, w: str) -> bool:
            if wi == len(w):
                return True
            le = w[wi]
            for j in range(bisect_left(idx_map[le], si), len(idx_map[le])):
                if is_match(wi + 1, idx_map[le][j] + 1, w):
                    return True
            return False
        ...
        for w in words:
            if len(w) > len(s):
                continue
            cw = Counter(w)
            if any(len(idx_map[le]) < cw[le] for le in cw):
                continue
            if not is_match(0, 0, w):
                continue
            res += 1
        return res
...
class Solution2:
    def numMatchingSubseq(self, s: str, words: List[str]) -> int:
        ...
        Same basic idea as Solution1, but consider this: it will always be
        better to match the first available matching letter in s, because if
        we don't do it and we still have a match, then we will still match if
        we do match the first available. Hence, matching the first available is
        always a good strategy.

        Being greedy, it does not need recursion anymore. I got this solution
        last time, but not this time.

        O(N + WlogN), 845 ms, faster than 45.53%
        """
        N = len(s)
        idx_map = defaultdict(list)
        for i, le in enumerate(s):
        ...
        res = 0
        for w in words:
            boundary = -1  # available idx in s must be larger than boundary
            for le in w:
                if le not in idx_map:
                    break
                idx = bisect_right(idx_map[le], boundary)
                if idx == len(idx_map[le]):  # no more matching in s for le
                    break
                boundary = idx_map[le][idx]
            else:
                res += 1
        return res
...
tests = [
    ...
    ("", ["yjqqdvoxxxjbrccoaqqspqlsnxcnderaewsaqpkigtiqoqopth"], 0),
    ("", ["uccnuegvmkqtagudujuildlwefbyoywypakjrhiibrxdmsspjl"], 0),
]
